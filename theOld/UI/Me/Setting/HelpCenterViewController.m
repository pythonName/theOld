...
//handler name
static NSString *HelpCenterMessagehandler = @"HelpCenterMessagehandler";
...
- (void)registerMessageHandler
{
    ...
    [self.webView.configuration.userContentController addScriptMessageHandler:self name:HelpCenterMessagehandler];
}
...
- (BOOL)dealWithMessage:(WKScriptMessage *)message
{
    ...
    if ([message.name isEqualToString:HelpCenterMessagehandler] && [self.message.methodName isEqualToString:business1]) {
        [self business1];
        deal = YES;
